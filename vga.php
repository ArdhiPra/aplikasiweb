...
<head>
    ...
    <title>YourPC.id - Best Seller VGA</title>
...
<?php 
session_start();
include 'navbar/vganav.php';
include 'koneksi.php'; // File koneksi database
?>
<div class="main-content">
<div class="sidebar">
    <h1>Filter VGA</h1>
    <form method="GET" action="vga.php">

        <!-- Dropdown Tipe VGA -->
        <?php
        // Ambil nilai enum dari kolom tipe_barang khusus VGA
        ...
        if ($result_enum_tipe) {
            ...
            $enum_values_tipe = $row_enum_tipe['Type']; // Hasilnya: enum('Nvidia', 'AMD', ...)
            preg_match("/^enum\((.*)\)$/", $enum_values_tipe, $matches_tipe);
            $values_tipe = str_getcsv($matches_tipe[1], ',', "'");
        }
        ?>
        <label for="tipe_barang">Tipe VGA:</label>
        ...
        ?>
        <label for="brand_barang">Brand VGA:</label>
        ...
        <!-- Dropdown VGA Memory (filter_barang) -->
        <?php
        // Ambil nilai enum dari kolom filter_barang (misalnya kapasitas memori)
        ...
        if ($result_enum) {
            ...
            $enum_values = $row_enum['Type']; // Hasilnya: enum('4GB', '6GB', '8GB', ...)
            preg_match("/^enum\((.*)\)$/", $enum_values, $matches);
            $values = str_getcsv($matches[1], ',', "'");
        }
        ?>
        <label for="filter_barang">Memory VGA:</label>
    ...
    <div class="product-section">
        ...
        // Filter berdasarkan filter_barang (memori VGA)
        ...
        if (!empty($_GET['min_price'])) {
            ...
            $conditions[] = "harga_barang >= $min_price";
        }
        if (!empty($_GET['max_price'])) {
            $max_price = (int) $_GET['max_price'];
            $conditions[] = "harga_barang <= $max_price";
        }

        // Gabungkan kondisi tambahan ke query dasar
        if (!empty($conditions)) {
            $sql .= " AND " . implode(" AND ", $conditions); // Menggunakan AND karena kategori_barang sudah menjadi bagian dari WHERE
        }
